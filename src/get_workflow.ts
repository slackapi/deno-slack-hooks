...
const readFile = async (path: string) => {
  try {
    ...
    if (e instanceof Deno.errors.NotFound) {
      throw new Error("DefaultExport Definition file cannot be found");
    }
    throw e;
  }
};
...
const readTSFile = async (path: string) => {
  try {
    ...
    if (file && !file.default) {
      throw new Error(
        "Nothing is being exported by default",
      );
    }
    //console.log("file: ", file.default.definition);
    return file.default.definition;
  } catch (e) {
    throw e;
  }
};
...
if (import.meta.main) {
  console.log(JSON.stringify(await getDefaultExport()));
}
